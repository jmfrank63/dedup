...
// char *read_and_free_ring_buffer(RingBuffer *buffer, const struct timespec *timeout) {
//     pthread_mutex_lock(&buffer->mutex);
...
char *read_and_free_ring_buffer(RingBuffer *buffer, const struct timespec *timeout) {
    ...
    // Wait until there's an element to read
    while (!buffer->full && (buffer->start == buffer->end)) {
